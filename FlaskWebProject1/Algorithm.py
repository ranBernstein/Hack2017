...
def evaluate_model_line(sen, sender_name, authToken, model_file='model.dat'):
    ...
    if containsMeetingWords(sen):
        timeWords, personWords, geoWords = parseMeetingDetails(sen, chunker)
        meeting_dt, status = getMeetingDateTime(sen, timeWords)
        personDetails = getPersonDetails(personWords.append(sender_name))
        geo_st = ','.join(geoWords)
...
if __name__ == "__main__":
    ...
    if train:
        ...
        '''content= ["Let's meet tomorrow at 13:00?",
                  "Let's meet next week.",
                  "Do you want to meet at 15:00?",
                  "Do you want to meet yesterday?",
                  "How was the meeting at 15:00?",
                  "Do you want to go to lunch at 14:00 at Zozobra?",
                  "Let's meet with Ran in an hour.",
                  "Do you want to sit at 15:00 at your place?",
                  "Shall we meet in an hour with Ran?",
                  "Do you want to meet now?",
                  "Do you want to go somewhere tomorrow at 19:00?",
                  "Please schedule a presentation of the project tomorrow",
                  "Let's meet after work for a beer.",
                  "Do you want to talk about the new project before lunch?"]
        for line in content:
            #Test the model
            print chunker.parse(pos_tag(word_tokenize(line)))'''